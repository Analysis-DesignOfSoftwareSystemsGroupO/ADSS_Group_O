    ...
    public class ShiftController implements IShiftController {
         ...
         */
        public ShiftController(BranchDTO Branch, IRoleController dependency, ITransportController transportController) throws SQLException {
            ...
            this.transportController = transportController; // חדש
        }
        ...
        public void assignEmployeeToShift(UserDTO theCaller, ShiftDTO theShift, EmployeeDTO theEmployee, RoleDTO theRole) throws SQLException {
            ...
            // DB
            curBranch.getWeekRepo().insertEmployeeToShift(
            ...
            // ======= 🚚 NEW: If role is Driver, assign to transport ==========
            if (role.getDescription().equalsIgnoreCase("Driver")) {
                try {
                    ...
                    for (TransportDTO t : transports) {
                        ...
                        if (t.getSiteName().equalsIgnoreCase(curBranch.getName())
                                && shift.getDay() == day
                                && shift.getType() == type) {

                            transportController.assignDriverTransport(
                                    String.valueOf(employee.getEmpId()),
                                    String.valueOf(t.getId())
                            );

                            System.out.printf("✅ Driver %s assigned to transport %s%n", employee.getEmpName(), t.getId());
                        }
                    }

                } catch (Exception e) {
                    System.out.println("⚠ Failed to assign driver to transport: " + e.getMessage());
                }
            }
        }
